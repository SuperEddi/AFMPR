...
function App() {
    ...
    const [reporteTipo, setReporteTipo] = useState(null); // null = dashboard visible
        ...
        <div className="flex min-h-[100dvh] bg-slate-50 overflow-x-hidden w-full">
            ...
            <main className={`flex-1 min-w-0 w-full max-w-[100vw] overflow-x-hidden transition-all duration-300 ${isSidebarOpen ? 'blur-sm pointer-events-none' : ''} lg:ml-64 p-2 sm:p-3 md:p-6`}>
                    ...
                    <div className="flex items-center gap-2 md:gap-3">
                        ...
                        <div className="hidden md:flex items-center gap-2">
                            <div className="flex bg-slate-100 p-1.5 rounded-xl border-2 border-indigo-200 shadow-inner">
                                <button
                                    onClick={() => { setInstitution('tierras'); localStorage.setItem('selectedInstitution', 'tierras'); window.dispatchEvent(new CustomEvent('data-updated')); }}
                                ...
                                >TIERRAS</button>
                                <button
                                    onClick={() => { setInstitution('justicia'); localStorage.setItem('selectedInstitution', 'justicia'); window.dispatchEvent(new CustomEvent('data-updated')); }}
                                ...
                                >JUSTICIA</button>
                                <button
                                    onClick={() => { setInstitution('presidencia'); localStorage.setItem('selectedInstitution', 'presidencia'); window.dispatchEvent(new CustomEvent('data-updated')); }}
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <div className="w-9 h-9 bg-blue-600 text-white rounded-full flex items-center justify-center font-black text-sm shadow-lg shadow-blue-500/20">AD</div>
                        </div>
                    </div>
                </header>
                ...
                <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                    ...
                    {activeTab === 'usuarios' && <UsuariosView authFetch={authFetch} />}
                    {activeTab === 'activos' && <ActivosView authFetch={authFetch} />}
                    {activeTab === 'migraciones' && <MigracionView authFetch={authFetch} />}
                    {activeTab === 'generar' && <GenerarActaView authFetch={authFetch} />}
